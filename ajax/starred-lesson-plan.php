<?php
	...
	if(isset($_POST["uniqueid"]) && $_POST["uniqueid"] != ""){
		...
		if(strpos($uId, ",") === false){
			if($_POST["value"] == 0){
				$seltopic = "SELECT topic_id FROM avn_user_topic_starred WHERE topic_id = " . $uId;
				$seltopicRS = $db->query("query", $seltopic);
				if(!array_key_exists("response", $seltopicRS)){
					$topictodelte = array();
					$topictodelte["topic_id"] = $uId;
					$deltopic = $db->delete("avn_user_topic_starred",$topictodelte);
			...
			if($_POST["value"] == 1){
				...
				$intopic = $db->insert("avn_user_topic_starred",$datatoSave);
				...
				for($i = 0; $i < count($allUID); $i++){
					if($_POST["value"] == 0){
						$seltopic = "SELECT topic_id FROM avn_user_topic_starred WHERE topic_id = " .  $allUID[$i];
						$seltopicRS = $db->query("query", $seltopic);
						if(!array_key_exists("response", $seltopicRS)){
							$topictodelte = array();
