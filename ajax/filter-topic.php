<?php
	...
	if(isset($_GET["currid"])||isset($_POST["chkgrade"]) || isset($_POST['chkcurriculum']) || isset($_POST['maxtpid']) || isset($_POST['chpID'])){
		...
		if(!isset($_POST['maxtpid'])){
			$order = " AND tm.status = 1 ORDER BY tm.unique_id DESC LIMIT 0,4";
		}
		else{ 
		...
		if(count($r) > 0){
			...
			if(!isset($r['response'])){
				...
				for($i=0; $i<count($r); $i++){
					...
					$html.="</td>";
					$html.="<td valign='top'><div class='ffhelvetica'><span class='enterlessonsdiv'><a href='" . __WEBROOT__ . "/" . trim($curriculum) . "/" . trim($r[$i]["category_slug"]) . "/" . trim($r[$i]["chapter_slug"]) . "/" . trim($r[$i]["topic_slug"]) . "/' >" . $r[$i]['topic_code'] . " - ".ucwords($r[$i]['topic_name'])."</a></span></div>";
					$desc = $r[$i]['topic_desc'];
					...
					if(!isset($r['response']))
					{
						...
						$html.="<span class='fl' style='font-family:helvetica;color:#333;'><small>". $time . "&nbsp;|&nbsp;". $Htime . "</small></span>";
					}
					$html.="</td>";
					$html.="<td rowspan='2'>";
					$html.="<span class='fr' style='margin-right:30px;'><a href='" . __WEBROOT__ . "/" . trim($curriculum) . "/" . trim($r[$i]["category_slug"]) . "/" . trim($r[$i]["chapter_slug"]) . "/" . trim($r[$i]["topic_slug"]) . "/' class='fl' ><img src='" . __WEBROOT__ . "/images/arrow.png' class='fr'></a></span>";
					$html.="</td>";
					$html.="</tr>";
					$html.="<tr>";
					$html.="<td colspan='4'>";
