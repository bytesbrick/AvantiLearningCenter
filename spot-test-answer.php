...
<title>Avanti Learning Centres | Spot Test Answer</title>
<?php
	...
	<div class="clb"></div>
		...
		<div class="whole">
				...
				<div class="ansoption">
					...
					?>
						...
						<tr>
							...
							<?php
							    //$count = $db->query("query"," select MAX(qm.unique_id) as maxuid from avn_question_master qm inner join ltf_resources_master rm on rm.unique_id = qm.resource_id where type=2 and rm.topic_id=". $_GET['tpID']);
						...
						<tr>
						<?php
								    //$sc = $db->query("query","SELECT SUM(is_correct) as score from user_test_answer ua inner join avn_question_master qm on qm.unique_id = ua.test_id where qm.type = 2 and  ua.user_id = '" .$userid . "'");
			...
			<div class="resource-pic">
			    <img src="./admin/images/upload-image/<?php echo $r[0]["resource_pic"]; ?>" width="294" height="150"  />
			</div>
			<div class="resource">
				...
				?>
					...
					$cur= $db->query("stored procedure","res_cur_select(".$_GET['tpID'] .")");
				    ?>
					<div class="resdta">
						...
						for($j=0;$j<count($cur);$j++){
						    if($j<count($cur)-1){
					    ?>
							<span class="headtext"><?php echo $cur[$j]['curriculum_name']; ?>,</span>
					    <?php
						}else{
					    ?>
							<span class="headtext"><?php echo $cur[$j]['curriculum_name']; ?></span>
					...
					$gr= $db->query("stored procedure","res_grd_select(".$_GET['tpID'].")");
				    ?>
					<div class="resdta">
						...
						for($j=0;$j<count($gr);$j++){
						   if($j<count($gr)-1){
					    ?>
							    <span class="headtext"><?php echo $gr[$j]['grade_name']; ?>,</span>
					    <?php
						    }else{
					    ?>
							    <span class="headtext"><?php echo $gr[$j]['grade_name']; ?></span>
					...
					<div class="resdta">
					    ...
					    <span class="headtext"><?php echo $r[0]['tag_name']; ?></span>
					</div>
				    <?php
				    }else{
				    ?>
