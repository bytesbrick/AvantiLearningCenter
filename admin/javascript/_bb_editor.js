var bsrName = "";
var prjPath = "http://la.bytesbrick.com/admin/images/bb-editor";
//var prjPath = "http://www.doctorlifeline.com/dll-admin/images/bb-editor";
function _showEditor(edNameId, w, h, htmlContent)
{
	...
	if(bsrName == "msie" || bsrName == "gecko")
	{
		...
		document.writeln("<iframe name=\"" + edNameId + "\" id=\"" + edNameId + "\" width=\"" + w + "px\" height=\"" + h + "px\" style=\"margin-top:5px\"></iframe>");
		document.writeln("<input type=\"hidden\" name=\"txtMailerBody\" id=\"txtMailerBody\" value=\"\">");
		document.getElementById("txtMailerBody").value = "";
		...
		var defaultContent = "<html><head></head><body style=\"font-family: verdana; font-size:13px;\">" + htmlContent + "</body></html>";
...
function _executeCommand(edNameId, command, option, arrPNames, arrPValues)
{
	...
	try{
		...
		iFrameSrc.document.execCommand(command, false, option);
		iFrameSrc.focus();
...
function _showImageUpload()
{
	window.open('./image-url-upload.php?type=image','imgUpload','width=520px,height=500px,toolbar=no,menubar=no,scrollbars=yes');
};

function _showImages(edNameId, command, option)
{
	if(bsrName == "msie")
	{
		var imgURL = prompt("Type in the image path/URL", "http://");
		if(imgURL != "http://")
			_executeCommand(edNameId, 'insertimage', imgURL);
	}
	else
	{
		_disableThisPage();
		try{
			hLinkDiv = document.createElement("<div id=\"img_" + edNameId + "\" style=\"background-color:#ffffff;z-index:100;top:0;left:0;position:absolute;width:550px;border:solid 2px #323232;\">");
		}
		catch(e)
		{
			hLinkDiv = document.createElement("div");
			hLinkDiv.id = "img_" + edNameId;
			hLinkDiv.style.backgroundColor = "#ffffff";
			hLinkDiv.style.zIndex = "100";
			hLinkDiv.style.top = 0;
			hLinkDiv.style.left = 0;
			hLinkDiv.style.position = "absolute";
			hLinkDiv.style.width = "550px";
			hLinkDiv.style.border = "solid";
			hLinkDiv.style.borderWidth = "2px";
			hLinkDiv.style.borderColor = "#323232";
		}
		var hLinkHTML = "<table width=\"300px\" cellpacing=\"2\" cellpadding=\"3\" border=\"0\">";
		hLinkHTML += "<tr><td><span style=\"font-family: verdana; font-size:12px\">Type in the image path/URL</span></td></tr>";
		hLinkHTML += "<tr><td><input type=\"text\" name=\"txtImgURL\" id=\"txtImgURL\" class=\"textbox\" size=\"70\"></td></tr>";
		hLinkHTML += "<tr><td><input type=\"button\" name=\"btnImgURL\" id=\"btnImgURL\" class=\"button\" value=\"Insert image\" onClick=\"javascript: if(document.getElementById('txtImgURL').value != '') {_executeCommand('" + edNameId + "', 'insertimage', document.getElementById('txtImgURL').value);}var divMe = document.getElementById('img_" + edNameId + "');divMe.style.display='none';document.body.removeChild(divMe);_enableThisPage();\">&nbsp;<input type=\"button\" name=\"btnClose\" id=\"btnClose\" class=\"button\" value=\"Close\" onClick=\"javascript: var divMe = document.getElementById('img_" + edNameId + "');divMe.style.display='none';document.body.removeChild(divMe);_enableThisPage();\"></td></tr>";
		hLinkHTML += "</table>";
		hLinkDiv.innerHTML = hLinkHTML;
		...
		if(bsrName == "msie")
		{
			try{
				...
				currY = document.documentElement.scrollTop;
			}
		}
		else
		{
		...
		document.body.appendChild(hLinkDiv);
		hLinkDiv.style.left = currX + (scrWidth/4) + "px";
		hLinkDiv.style.top = currY + parseInt(scrHeight/3) + "px";
	}
	
};
_showUploadOption = function(opt){
	if(opt == 1)
	{
...
function _showHyperlink(edNameId, command, option)
{
	...
	else
	{
		_disableThisPage();
		try{
			hLinkDiv = document.createElement("<div id=\"hLink_" + edNameId + "\" style=\"background-color:#ffffff;z-index:100;top:0;left:0;position:absolute;width:550px;border:solid 2px #323232;\">");
		}
		catch(e)
		{
			hLinkDiv = document.createElement("div");
			hLinkDiv.id = "hLink_" + edNameId;
			hLinkDiv.style.backgroundColor = "#ffffff";
			hLinkDiv.style.zIndex = "100";
			hLinkDiv.style.top = 0;
			hLinkDiv.style.left = 0;
		...
		var hLinkHTML = "<table width=\"300px\" cellpacing=\"2\" cellpadding=\"3\" border=\"0\">";
		hLinkHTML += "<tr><td><span style=\"font-family: verdana; font-size:12px\">Type in the URL</span></td></tr>";
		hLinkHTML += "<tr><td><input type=\"text\" name=\"txtHLink\" id=\"txtHLink\" class=\"textbox\" size=\"70\"></td></tr>";
		hLinkHTML += "<tr><td><input type=\"button\" name=\"btnHLink\" id=\"btnHLink\" class=\"button\" value=\"Insert link\" onClick=\"javascript: if(document.getElementById('txtHLink').value != '') {_executeCommand('" + edNameId + "', 'createLink', document.getElementById('txtHLink').value);}var divMe = document.getElementById('hLink_" + edNameId + "');divMe.style.display='none';document.body.removeChild(divMe);_enableThisPage();\">&nbsp;<input type=\"button\" name=\"btnClose\" id=\"btnClose\" class=\"button\" value=\"Close\" onClick=\"javascript: var divMe = document.getElementById('hLink_" + edNameId + "');divMe.style.display='none';document.body.removeChild(divMe);_enableThisPage();\"></td></tr>";
		hLinkHTML += "</table>";
		hLinkDiv.innerHTML = hLinkHTML;
