...
<head>
<title>Avanti &raquo; Resources &raquo; Spot Test Questions</title>
<?php
	include_once("./includes/head-meta.php");
?>
<script type="text/javascript" language="javascript" src="./javascript/spot-test-questions.js"></script>
</head>
<body>
<div id="page">
		...
		<div id="wrapper">	
			...
			<div id="container2">
				<div class="about2">
					...
					<select class="inputseacrh inputseacrh2" onchange="javascript: window.location='./chapter.php?catgid=' + this.value;"><option value=''>All</option><?php for($sb = 0; $sb < count($rsSubjects); $sb++){ if($rsSubjects[$sb]["unique_id"] == $catgid){echo "<option value='" . $rsSubjects[$sb]["unique_id"] . "' selected='selected'>" . $rsSubjects[$sb]["category_name"] . "</option>";} else {echo "<option value='" . $rsSubjects[$sb]["unique_id"] . "'>" . $rsSubjects[$sb]["category_name"] . "</option>";}} ?>
					</select>
					<span class="mt2px fl">&nbsp;&raquo;&nbsp;</span>
					<select class="inputseacrh inputseacrh2" onchange="javascript: window.location='./topic.php?catgid=<?php echo $catgid; ?>&chptid=' + this.value;"><option value=''>All</option><?php for($ch = 0; $ch < count($rsChapters); $ch++){ if($rsChapters[$ch]["unique_id"] == $chptid){echo "<option value='" . $rsChapters[$ch]["unique_id"] . "' selected='selected'>" . $rsChapters[$ch]["chapter_name"] . "</option>";} else {echo "<option value='" . $rsChapters[$ch]["unique_id"] . "'>" . $rsChapters[$ch]["chapter_name"] . "</option>";}} ?></select>
					<span class="mt2px fl">&nbsp;&raquo;&nbsp;</span>
					<select class="inputseacrh inputseacrh2" onchange="javascript: var d = this.value.split('|'); window.location='./resource.php?chptid=<?php echo $chptid; ?>&rid=' + d[0] + '&catgid=<?php echo $catgid; ?>&topicid=' + d[1];"><option value=''>All</option><?php for($tp = 0; $tp < count($rstopics); $tp++){ if($rstopics[$tp]["unique_id"] == $topicid){echo "<option value='" . $rstopics[$tp]["ResID"] . "|" . $rstopics[$tp]["unique_id"] ."' selected='selected'>" . $rstopics[$tp]["topic_name"] . "</option>";} else {echo "<option value='" . $rstopics[$tp]["ResID"] . "|" . $rstopics[$tp]["unique_id"] . "'>" . $rstopics[$tp]["topic_name"] . "</option>";}} ?></select>
					<span class="mt2px fl">&nbsp;&raquo;&nbsp;<a href="./resource.php?currid=<?php echo $currid; ?>&chptid=<?php echo $chptid; ?>&catgid=<?php echo $catgid; ?>&topicid=<?php echo $topicid; ?>">Resources</a></span>
					<span class="mt2px fl">&nbsp;&raquo;&nbsp;Spot Test</span>
				</div>
				<div class="act"><a href="./add-spot-test.php<?php echo filter_querystring($_SERVER["QUERY_STRING"], array("cid","type","resp","page"), array("","1","","")); ?>">Add Question</a>
					<div id="bulkAct"><a href="javascript:void(0);" id="btnDel" onclick="javascript: _deletequestion(<?php echo $currid; ?>,-1,<?php echo $topicid; ?>,<?php echo $chptid; ?>,<?php echo $catgid;?>,'1',<?php echo $curPage; ?>);" class="ml5">Delete</a>
					</div>
					<div id="bulkActive"><a href="javascript:void(0);" id="activestatus" onclick="javascript: _chngQesStatus(-1,<?php echo $topicid; ?>,<?php echo $chptid; ?>,<?php echo $catgid; ?>,this.id,1,<?php echo $curPage; ?>);" class="ml5">Active</a>
					</div>
				</div><!-- End of about -->
				<?php
				...
				<div id="ErrMsg" style="display: block;">
					<?php
						if($_SESSION["resp"] == "err")
							...
							echo "Spot test question is successfully added.";
						else if($_SESSION["resp"] == "Errdlt")
							echo "Spot test question is not deleted successfully.";
						else if($_SESSION["resp"] == "sucdt")
							echo "Spot test question successfully deleted.";
						else if($_SESSION["resp"] == "up")
							echo "Spot test question edited successfully.";
						else if($_SESSION["resp"] == "invEd")
							echo "Spot test question can't be saved.";
						else if($_SESSION["resp"] == "invUp")
							echo "Spot test question can't be updated because this is already exist.";
...
<script type="text/javascript"> 
	_getSpotQesTable(<?php echo $currid; ?>,<?php echo $topicid; ?>, <?php echo $catgid; ?>, <?php echo $chptid; ?>,<?php echo $curPage; ?>,'','')
</script>
</body>
</html>
<?php
